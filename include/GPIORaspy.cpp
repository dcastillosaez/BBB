...
GPIORaspy::GPIORaspy(pinGPIORaspy pinName_p)
{
    this->debounceTime = 0;
...
int GPIORaspy::exportGPIO()
{
   return write(GPIO_PATH, "export", this->number);
}

int GPIORaspy::unexportGPIO()
{
   return write(GPIO_PATH, "unexport", this->number);
}

int GPIORaspy::setDirection(GPIORaspy::DIRECTION dir)
{
   switch(dir){
...
int GPIORaspy::setValue(GPIORaspy::VALUE value)
{
   switch(value){
...
int GPIORaspy::setEdgeType(GPIORaspy::EDGE value)
{
   switch(value){
...
GPIORaspy::DIRECTION GPIORaspy::getDirection()
{
...
GPIORaspy::EDGE GPIORaspy::getEdgeType()
{
...
int GPIORaspy::streamOpen()
{
    stream.open((path + "value").c_str());
    return 0;
}
int GPIORaspy::streamWrite(GPIORaspy::VALUE value)
{
    stream << value << std::flush;
    return 0;
}
int GPIORaspy::streamClose()
{
    stream.close();
    return 0;
}

int GPIORaspy::toggleOutput()
{
...
int GPIORaspy::toggleOutput(int time)
{
    return this->toggleOutput(-1, time);
}
int GPIORaspy::toggleOutput(int numberOfTimes, int time)
{
    this->setDirection(OUTPUT);
...
void* threadedToggle(void *value){
    GPIORaspy *gpio = static_cast<GPIORaspy*>(value);
    bool isHigh = (bool) gpio->getValue(); //find current value
    while(gpio->threadRunning){
        if (isHigh)	gpio->setValue(GPIORaspy::HIGH);
        else gpio->setValue(GPIORaspy::LOW);
...
int GPIORaspy::waitForEdge(){
    ...
    if (epollfd == -1) {
       perror("GPIORaspy: Failed to create epollfd");
       return -1;
    }
    if ((fd = open((this->path + "value").c_str(), O_RDONLY | O_NONBLOCK)) == -1) {
       perror("GPIORaspy: Failed to open file");
       return -1;
    }
    ...
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, fd, &ev) == -1) {
       perror("GPIORaspy: Failed to add control interface");
       return -1;
    }
    //Wait for event
    long currentTime;
...
int GPIORaspy::waitForEdge(CallbackType callback){
    this->threadRunning = true;
